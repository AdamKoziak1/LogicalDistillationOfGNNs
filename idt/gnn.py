...
class GNN(LightningModule):
    def __init__(self, num_features, num_classes, layers, dim, conv="GCN", activation="ReLU", pool="mean", lr=1e-4, weight=None):
        from idt.ui import DBG, HLT, NEW, NXT, END, MRK, HLN, CND, ERR, PLT
        NEW("STARTING :: gnn.py -> class GNN -> __init__()")
        super().__init__()
        ...
        #NXT("layers = " + str(layers))
        NXT("convolution layers = " + str(self.conv_layers).split(":")[1].split("\n")[0])
        #NXT("convolution layers = " + str(self.conv_layers))
        NXT("activation = " + activation)
        self.out = Sequential(
            Linear(dim, dim),
            self.act,
            Linear(dim, num_classes)
        )
        #NXT("sequential layers = " + str(self.out))
    ...
    def training_step(self, batch, batch_idx):
        from idt.ui import DBG, HLT, NEW, NXT, END, MRK, HLN, CND, ERR, PLT
        #print("\n")
        #NEW("STARTING :: gnn.py -> class GNN -> training_step()")
        out = self(batch)
        ...
        #NXT("training step loss = " + str(loss).split("(")[1].split(",")[0])
        #END("COMPLETED :: gnn.py -> class GNN -> training_step()")
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        from idt.ui import DBG, HLT, NEW, NXT, END, MRK, HLN, CND, ERR, PLT
        #print("\n")
        #NEW("STARTING :: gnn.py -> class GNN -> validation_step()")
        out = self(batch)
        loss = self.loss(out, batch.y)
        acc = (out.argmax(dim=1) == batch.y).float().mean().item()
        f1_macro = f1_score(batch.y.cpu(), out.argmax(dim=1).cpu(), average='macro')
        ...
        #NXT("validation step loss = " + str(loss).split("(")[1].split(",")[0])
        #END("COMPLETED :: gnn.py -> class GNN -> validation_step()")
        return loss
