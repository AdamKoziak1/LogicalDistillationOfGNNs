...
def create_and_train_GNN(num_features, num_classes, weight, args, train_loader, val_loader, logger, device, conv="GCN"):
    ...
    early_stop_callback = EarlyStopping(monitor=f"{conv}_val_loss", patience=10, mode="min")
    trainer = Trainer(
...
def run_split(args, cv_split, run_id, device=0):
    ...
    for j, conv_type in enumerate(conv_types):
        ...
        logger.experiment.summary[f'idt_true_{conv_type}_fidelity'] = fidelity
        print(f"   idt_true_{conv_type}_fidelity: {fidelity}")
